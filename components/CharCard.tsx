...
export function CharCard({ entry }: Props) {
  return (
    <Link
      href={`/char/${encodeURIComponent(entry.char)}`}
      className="flex flex-col items-center gap-1 px-4 py-4 bg-white border border-[#E0E0DC] rounded-xl hover:border-[#0F0F0F] transition-all shadow-sm active:scale-[0.97] cursor-pointer"
    >
